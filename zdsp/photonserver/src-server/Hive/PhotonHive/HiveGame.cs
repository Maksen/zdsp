...
namespace Photon.Hive
{
    ...
    public class HiveGame : Room
    {
        ...
        public HiveGame(string gameName, RoomCacheBase roomCache, IGameStateFactory gameStateFactory = null, 
            int maxEmptyRoomTTL = 0, int lastTouchLimitMilliseconds = 0, int lastTouchCheckIntervalMilliseconds = 0, PoolFiber executionFiber = null)
        ...
        protected void PublishEventCache(HivePeer HivePeer, JoinGameRequest joinRequest)
        {
            ...
            foreach (KeyValuePair<int, EventCache> entry in this.ActorEventCache)
            {
                ...
                foreach (KeyValuePair<byte, Hashtable> eventEntry in cache)
                {
                    ...
                    HivePeer.SendEvent(eventData, new SendParameters());
                }
            }
            ...
            foreach (var slice in this.EventCache.Slices)
            {
                if (slice >= cacheSliceRequested)
                {
                    if (slice != 0)
                    {
                        ...
                        this.PublishEvent(sliceChangedEvent, this.GetActorByPeer(HivePeer), new SendParameters());
                    }
