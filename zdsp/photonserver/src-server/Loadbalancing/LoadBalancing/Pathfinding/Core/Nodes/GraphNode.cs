...
namespace Pathfinding {
	...
	public abstract class GraphNode {
		...
		protected GraphNode (AstarPath astar, AstarData ad) {
            level = ad.level;
			if (!System.Object.ReferenceEquals (astar, null)) {
				this.nodeIndex = astar.GetNewNodeIndex();
				astar.InitializeNode (this, ad.level);
			} else {
		 ...
		 */
		public void Destroy (string level ) {
			//Already destroyed
			if (Destroyed) return;

			ClearConnections(level, true);
...
#region Properties
		...
		public virtual void UpdateRecursiveG (string level, Path path, PathNode pathNode, PathHandler handler) {
			//Simple but slow default implementation
			UpdateG (path,pathNode);

			handler.PushNode (pathNode);

			GetConnections (level, delegate (GraphNode other) {
				PathNode otherPN = handler.GetPathNode (other);
				if (otherPN.parent == pathNode && otherPN.pathID == handler.PathID) other.UpdateRecursiveG (level, path, otherPN,handler);
			});
		}

		public virtual void FloodFill (string level, Stack<GraphNode> stack, uint region) {
			//Simple but slow default implementation
		...
		public abstract void GetConnections (string level, GraphNodeDelegate del);
		...
		/** Remove all connections from this node.
		  ...
		  */
		public abstract void ClearConnections (string level, bool alsoReverse);

		/** Checks if this node has a connection to the specified node */
		public virtual bool ContainsConnection (GraphNode node) {
			//Simple but slow default implementation
			bool contains = false;
			GetConnections (level, delegate (GraphNode n) {
		 ...
		 */
		public virtual bool GetPortal (string level, GraphNode other, List<Vector3> left, List<Vector3> right, bool backwards) {
			return false;
		}

		/** Open the node */
		public abstract void Open (string level, Path path, PathNode pathNode, PathHandler handler);
    ...
    //    protected MeshNode (AstarPath astar, AstarData ad) : base (astar, ad) {
    //    }
