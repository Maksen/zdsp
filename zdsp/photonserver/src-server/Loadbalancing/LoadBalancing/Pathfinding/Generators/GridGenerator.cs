...
\nosubgrouping
	...
	public class GridGraph : NavGraph, IUpdatableGraph
	{
        ...
        void RemoveGridGraphFromStatic()
        {
            ...
            GridNode.SetGridGraph(astarData.level, astarData.GetGraphIndex(this), null);
        }
        ...
        public static Quaternion ZXYEulerToQuaternion(float rx, float ry, float rz)
        {
            Quaternion res;
            Quaternion qx = Quaternion.identity, qy = Quaternion.identity, qz = Quaternion.identity;
            qx.w = (float)System.Math.Cos(0.5 * rx); qx.x = (float)System.Math.Sin(0.5 * rx);
            qy.w = (float)System.Math.Cos(0.5 * ry); qx.y = (float)System.Math.Sin(0.5 * ry);
            qz.w = (float)System.Math.Cos(0.5 * rz); qx.z = (float)System.Math.Sin(0.5 * rz);
            res = qx * qy;
            res = qz * res;
            return res;
        }

        public static Matrix4x4 ZXYEulerToRotationMatrix(float rx, float ry, float rz)
        {
            Matrix4x4 res = Matrix4x4.identity;
            float s1, s2, s3, c1, c2, c3;
            s1 = (float)System.Math.Sin(rz);
            s2 = (float)System.Math.Sin(rx);
            s3 = (float)System.Math.Sin(ry);
            c1 = (float)System.Math.Cos(rz);
            c2 = (float)System.Math.Cos(rx);
            c3 = (float)System.Math.Cos(ry);

            res[0, 0] = c1*c3-s1*s2*s3; res[0, 1] = -c2*s1; res[0, 2] = c1*s3+c3*s1*s2;
            res[1, 0] = c3*s1+c1*s2*s3; res[1, 1] = c1*c2; res[1, 2] = s1*s3-c1*c3*s2;
            res[2, 0] = -c2*s3; res[2, 1] = s2; res[2, 2] = c2*c3;
            return res;
        }
		...
		public void GenerateMatrix () {
            ...
            //Quaternion quat = ZXYEulerToQuaternion(0, 45, 0);
            Matrix4x4 isometricMatrix = ZXYEulerToRotationMatrix(0,45,0);            
                        
            //var isometricMatrix = Matrix4x4.TRS(Vector3.zero, EulerToQuaternionZXY(0, 45, 0), Vector3.one);
            ...
            isometricMatrix = ZXYEulerToRotationMatrix(0, -45, 0) * isometricMatrix;
            ...
            Matrix4x4 rotmat = ZXYEulerToRotationMatrix(rotation.x, rotation.y, rotation.z);
            Matrix4x4 mat = rotmat;
            mat[0, 0] = mat[0, 0] * aspectRatio;
            mat[0, 3] = center.x;
            mat[1, 3] = center.y;
            mat[2, 3] = center.z;
            boundsMatrix = mat * isometricMatrix;
            ...
            Vector3 trans = boundsMatrix.MultiplyPoint3x4 (-new Vector3 (size.x,0,size.y)*0.5F);
            ...
            Matrix4x4 m = rotmat;
            m[0, 0] = m[0, 0] * nodeSize * aspectRatio;
            m[2, 2] = m[2, 2] * nodeSize;
            m[0, 3] = trans.x;
            m[1, 3] = trans.y;
            m[2, 3] = trans.z;
		...
		public override NNInfo GetNearest (Vector3 position, NNConstraint constraint, GraphNode hint, Vector3? preferredDir) {
            ...
            {
                //choose position based on preferred direction
                Vector3 dir = (Vector3) preferredDir;
                if (dir.x > 0.5f)
                    xf += 0.5f;
                else if (dir.x < -0.5f)
                    xf -= 0.5f;
		...
		/** \todo Set clamped position for Grid Graph */
		public override NNInfo GetNearestForce (Vector3 position, NNConstraint constraint, Vector3? preferredDir) {
            ...
            float maxDist = constraint.constrainDistance ? active.maxNearestNodeDistance : float.PositiveInfinity;
		...
		public override void DeserializeExtraInfo (GraphSerializationContext ctx) {
			...
			for (int i=0;i<nodes.Length;i++) {
				nodes[i] = new GridNode (active,astarData);
		...
		public override void PostDeserialization () {
            ...
            GridNode.SetGridGraph(astarData.level, astarData.GetGraphIndex(this), this);
