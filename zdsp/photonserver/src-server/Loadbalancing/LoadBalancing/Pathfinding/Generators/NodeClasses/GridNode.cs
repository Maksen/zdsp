...
namespace Pathfinding {
	...
	public abstract class GridNodeBase : GraphNode {
		...
		protected GridNodeBase (AstarPath astar, AstarData ad) : base (astar, ad) {
		}
	...
	public class GridNode : GridNodeBase {
		...
		public GridNode (AstarPath astar, AstarData ad) : base (astar, ad) {
		}

        //private static GridGraph[] _gridGraphs = new GridGraph[0];
        private static Dictionary<string, GridGraph[]> _gridGraphs = new Dictionary<string, GridGraph[]>();
        public static GridGraph GetGridGraph(string level, uint graphIndex) { return _gridGraphs[level][(int)graphIndex]; } //error check?

		public GraphNode[] connections;
		public uint[] connectionCosts;

		public static void SetGridGraph (string level, int graphIndex, GridGraph graph) {
            if (!_gridGraphs.ContainsKey(level)) _gridGraphs.Add(level, new GridGraph[0]);
			if (_gridGraphs[level].Length <= graphIndex) {
				var gg = new GridGraph[graphIndex+1];
                for (int i = 0; i < _gridGraphs[level].Length; i++) 
                    gg[i] = _gridGraphs[level][i];
                _gridGraphs[level] = gg;
			}

            _gridGraphs[level][graphIndex] = graph;
		}
...
[7] = -Y-X
		...
		*/
		...
		public override void ClearConnections (string level, bool alsoReverse) {
			if (alsoReverse) {
				GridGraph gg = GetGridGraph (level, GraphIndex);
				for (int i=0;i<8;i++) {
		...
		public override void GetConnections (string level, GraphNodeDelegate del)
		{
		...
		public override bool GetPortal (string level, GraphNode other, List<Vector3> left, List<Vector3> right, bool backwards)
		{
		...
		public override void FloodFill (string level, Stack<GraphNode> stack, uint region) {
			GridGraph gg = GetGridGraph (level, GraphIndex);
			int[] neighbourOffsets = gg.neighbourOffsets;
			GridNode[] nodes = gg.nodes;
		...
		public override void UpdateRecursiveG (string level, Path path, PathNode pathNode, PathHandler handler) {
			GridGraph gg = GetGridGraph (level, GraphIndex);
			int[] neighbourOffsets = gg.neighbourOffsets;
			GridNode[] nodes = gg.nodes;
			...
			for (int i=0;i<8;i++) {
				if (GetConnectionInternal(i)) {
					...
					if (otherPN.parent == pathNode && otherPN.pathID == pid) other.UpdateRecursiveG (level, path, otherPN,handler);
				}
			}
		...
		public override void Open (string level, Path path, PathNode pathNode, PathHandler handler) {
					...
					if (GetConnectionInternal(i)) {
						...
						if (otherPN.pathID != pid) {
...
#endif
							{
								...
								other.UpdateRecursiveG (other.level, path,otherPN, handler);
...
#endif
							{
								...
								UpdateRecursiveG(other.level,path, pathNode, handler);
							}
						}
					}
				}
