...
//Mem - 4+1+4+1+[4]+[4]+1+1+4+4+4+4+4+  12+12+12+12+12+12+4+4+4+4+4+1+1+(4)+4+4+4+4+4+4+4 ? 166 bytes
	...
	public class ABPath : Path {
		...
		public override void Prepare () {
            ...
            NNInfo startNNInfo 	= AstarPath.active.GetNearest (startPoint,nnConstraint, startHint, preferredDir);
			...
			if (hasEndPoint) {
				NNInfo endNNInfo = AstarPath.active.GetNearest (endPoint,nnConstraint, endHint, null);
