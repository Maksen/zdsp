...
public class BacktraceReference : EditorWindow
    // from here: 
    // https://answers.unity.com/questions/155746/how-do-i-find-which-objects-are-referencing-anothe.html?page=1&pageSize=5&sort=votes
{
    ...
    /// </summary>
    [UnityEditor.MenuItem("GameObject/Find Objects Referencing This Selection", false, 48)]
    ...
    public void OnGUI()
    {
        ...
        foreach (var item in ReferencingSelection)
        {
            ...
            //Remove duplicates
            ReferencingSelection = ReferencingSelection.Distinct().ToList();
        }
    }
    ...
    private static Component[] GetAllActiveInScene()
    {
        ...
        {
            result.AddRange(rootObject.GetComponentsInChildren<Component>(includeInactive: true));
        }
    ...
    private static void componentReferences(Component component, Component references)
    {
        ...
        while (prop.NextVisible(true))
        {
            ...
            if (isObjectField && (prop.objectReferenceValue == references || prop.objectReferenceValue == references.gameObject))
            {
                if(component.name != Selection.activeObject.name)
                {
