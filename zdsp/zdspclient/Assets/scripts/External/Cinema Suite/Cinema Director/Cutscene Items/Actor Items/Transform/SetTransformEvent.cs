    ...
    public class SetTransformEvent : CinemaActorEvent, IRevertable
    {
        ...
        public override void Trigger(GameObject actor)
        {
            if (actor != null)
            {
                ...
                if(PhotonNetwork.connected && actor == GameInfo.gLocalPlayer.AnimObj)
                {
                    GameInfo.gLocalPlayer.Position = actor.transform.position;
                    GameInfo.gLocalPlayer.Forward = actor.transform.forward;
                }
            }
        }
