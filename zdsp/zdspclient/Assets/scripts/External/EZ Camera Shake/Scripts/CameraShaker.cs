...
namespace EZCameraShake
{
    ...
    public class CameraShaker : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            for (int i = 0; i < cameraShakeInstances.Count; i++)
            {
                ...
                else if (c.CurrentState != CameraShakeState.Inactive)
                {
                    if (c.CanUpdate)
                    {
                        Vector3 amt = c.UpdateShake();
                        posAddShake += CameraUtilities.MultiplyVectors(amt, c.PositionInfluence);
                        rotAddShake += CameraUtilities.MultiplyVectors(amt, c.RotationInfluence);
                    }
                    else
                        c.UpdateElapsedTime();
                }
        ...
        /// <returns>A CameraShakeInstance that can be used to alter the shake's properties.</returns>
        public CameraShakeInstance ShakeOnce(float magnitude, float roughness, float fadeInTime, float fadeOutTime, float startTime = 0)
        {
            CameraShakeInstance shake = new CameraShakeInstance(magnitude, roughness, fadeInTime, fadeOutTime, startTime);
        ...
        /// <returns>A CameraShakeInstance that can be used to alter the shake's properties.</returns>
        public CameraShakeInstance ShakeOnce(float magnitude, float roughness, float fadeInTime, float fadeOutTime, Vector3 posInfluence, Vector3 rotInfluence, float startTime = 0)
        {
            CameraShakeInstance shake = new CameraShakeInstance(magnitude, roughness, fadeInTime, fadeOutTime, startTime);
