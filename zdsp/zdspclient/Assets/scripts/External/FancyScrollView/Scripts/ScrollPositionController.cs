        ...
        void Update()
        {
            ...
            if (autoScrollState.Enable)
            {
                ...
                else
                {
                    ...
                    position = Mathf.Lerp(dragStartScrollPosition, autoScrollState.EndScrollPosition, alpha);
                    UpdatePosition(position);
                    if (Mathf.Approximately(alpha, 1f))
                    {
                        int index = Mathf.RoundToInt(GetCircularPosition(autoScrollState.EndScrollPosition, dataCount));
                        autoScrollState.Reset();
                        ItemSelected(index);
                    }
                }
