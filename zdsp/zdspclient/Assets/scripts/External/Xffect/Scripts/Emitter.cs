    ...
    public class Emitter
    {
        ...
        protected int EmitByRate(float deltaTime)
        {
            ...
            if (Layer.EmitDuration > 0 && EmitterElapsedTime >= Layer.EmitDuration)
            {
                ...
                IsFirstEmit = false;
                mElapsedTime = 0f;
                if (Layer.Owner.MeshRendereObj.ContainsKey(Layer.name) && Layer.Owner.MeshRendereObj[Layer.name] != null)
                    Layer.Owner.MeshRendereObj[Layer.name].gameObject.SetActive(false);
            }
            if (EmitLoop == 0)
            {
                return 0;
