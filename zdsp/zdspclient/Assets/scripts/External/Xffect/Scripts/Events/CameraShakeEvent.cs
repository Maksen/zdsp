...
namespace Xft
{
	...
	public class CameraShakeEvent : XftEvent
	{
        ...
        {
			base.Reset();
            //SuspendFixFollow(false);
            
			//m_cameraShake.EarthQuakeToggled = false;
        }

        public void SuspendFixFollow(bool val)
        {
            //FollowCamera ff = MyCamera.gameObject.GetComponent<FollowCamera>();
            //if (ff)
            //    ff.Suspend(val);
        }

        private bool CanShakeCamera(bool localshake)
        {
            CharacterController cctrl = m_owner.GetComponentInParent<CharacterController>();
            if (cctrl != null)
            {
                if (localshake)
                    return cctrl.gameObject.tag == "LocalPlayer";
                else
                    return true;
            }
        ...
        public override void Update(float deltaTime)
        {
            ...
            if (!bStarted)
            {
                ...
                {
                    ZDSPCamera mainCam = Camera.main.gameObject.GetComponent<ZDSPCamera>();
                    if (mainCam != null)
                    {
                        // x, y, z point to the same value                
                        if (m_owner.CameraShakeType == XCameraShakeType.Spring)
                            mainCam.DoShake(m_owner.PositionStifness, m_owner.EndTime);
                    }
                }
            }            
        }
		...
		{
            if (!CanShakeCamera(m_owner.LocalPlayerShake))
                return;
